...
//create a pipe with name x, x must be a positive integer
//0 for success, 1 for nagative name, 2 if name is not unique, 3 
//if the allocation is unsuccessful  
extern int mkfifo( int x );

//Opens a pipe with name x, returns file descriptor.
//returns descriptor if successful, -1 if the pipe doesn't exist, 
//-2 if it is already occupied, -3 for non valid mode. 
extern int open( int x, int mode );

//Closes the pipe with name x, returns 0 if successful, 
//1 if pipe doesn't exist or 2 if the process does not connect to it 
extern int close( int x );

//Deallocates the pipe x, 0 for success, 1 for pipe doesn't exist, 
//2 for pipe not connected to process, 3 for pipe not deallicated on other side
extern int unlink( int x );
#endif
